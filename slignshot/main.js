...
function showObj(id){
	...
	if(b.type == "Moon")
  	...
  	if(b.type == "Asteroid")
  		bodyInfo += " <img src='/objects_images/asteroid.jpg' alt='Star' height='100' width='120'>";
...
function updateSystem(){
    ...
    for(var i = 0; i < bodies.length; i++){
        if(bodies[i].type === 'moon'){ 
            var planet = getPlanet(bodies, bodies[i].parentPlanetId);
            if(planet){
                var dx = bodies[i].x - planet.x;
                var dy = bodies[i].y - planet.y;
                var dist = Math.sqrt(dx*dx + dy*dy);
                var force = G * planet.m * bodies[i].m / (dist * dist);
                var nx = dx / dist;
                var ny = dy / dist;
                bodies[i].ax += nx * force / bodies[i].m;
                bodies[i].ay += ny * force / bodies[i].m;
            }
        }
    }

    for(var i = 0; i < bodies.length; i++){
        bodies[i].vx += bodies[i].ax * globalDt;
        bodies[i].vy += bodies[i].ay * globalDt;
        
        bodies[i].x += bodies[i].vx * globalDt;
        bodies[i].y += bodies[i].vy * globalDt;
